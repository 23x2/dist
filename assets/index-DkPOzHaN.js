(function(){const d=document.createElement("link").relList;if(d&&d.supports&&d.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))w(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const n of s.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&w(n)}).observe(document,{childList:!0,subtree:!0});function m(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function w(r){if(r.ep)return;r.ep=!0;const s=m(r);fetch(r.href,s)}})();const R=3;let _=[],L=[],y=document.getElementById("connectionPath"),P=null,I=0;function F(){const p=document.querySelectorAll("#logo circle, #logo path");L=Array.from(p).map(d=>d.getAttribute("style").match(/fill: ([^;]+)/)[1])}function A(p,d,m){return p*(1-m)+d*m}function D(p){y.clientWidth;const d=y.clientHeight,w=document.getElementById("logo").getBoundingClientRect(),r=w.left+w.width/2;let s=Array(5).fill().map(()=>[]);for(let n=0;n<R;n++){const z=(n-R/2+.5)*10,x=r+z,f=r+z,u=5,v=150*(1+Math.sin(p*Math.PI)),b=1.5+p*2,g=p*Math.PI*2+n*Math.PI/R,i=[];for(let t=0;t<u;t++){const o=t/(u-1),a=Math.sin(o*Math.PI*b+g)*v,c=Math.cos(o*Math.PI*(b*1.5)+g)*(v*.5),l=A(x,f,o)+a*.5,S=A(0,d,o)+a+c;i.push({x:l,y:S}),s[t].push({x:l,y:S})}i[0].y=0,i[i.length-1].y=d;let h=`M ${i[0].x},${i[0].y}`;for(let t=0;t<i.length-1;t++){const o=i[t],a=i[t+1],c=(o.x+a.x)/2,l=(o.y+a.y)/2;h+=` Q ${o.x},${o.y} ${c},${l}`}h+=` L ${i[i.length-1].x},${i[i.length-1].y}`;let e=_[n];e||(e=document.createElementNS("http://www.w3.org/2000/svg","path"),_[n]=e,y.appendChild(e)),e.setAttribute("d",h),e.setAttribute("stroke",L[n%L.length]),e.setAttribute("fill","none"),e.setAttribute("stroke-width","1")}s.forEach((n,z)=>{if(n.length<2)return;let x=`M ${n[0].x},${n[0].y}`;for(let u=1;u<n.length-1;u++){const v=n[u-1],b=n[u],g=n[u+1],i=b.x-(g.x-v.x)*.2,h=b.y-(g.y-v.y)*.2,e=b.x+(g.x-v.x)*.2,t=b.y+(g.y-v.y)*.2;x+=` C ${i},${h} ${e},${t} ${g.x},${g.y}`}let f=y.querySelector(`#connecting-${z}`);f||(f=document.createElementNS("http://www.w3.org/2000/svg","path"),f.id=`connecting-${z}`,y.appendChild(f)),f.setAttribute("d",x),f.setAttribute("stroke",L[z%L.length])})}function U(){const p=window.scrollY/(document.documentElement.scrollHeight-window.innerHeight),m=A(I,p,.1);D(m),I=m,P=requestAnimationFrame(U)}F();y.style.position="fixed";y.style.top="0";y.style.left="0";y.style.pointerEvents="none";y.style.zIndex="1";U();window.addEventListener("pagehide",()=>{P&&cancelAnimationFrame(P)});(()=>{const m=`
    let gl, program;

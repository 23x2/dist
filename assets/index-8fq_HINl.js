(function(){const l=document.createElement("link").relList;if(l&&l.supports&&l.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))w(n);new MutationObserver(n=>{for(const c of n)if(c.type==="childList")for(const a of c.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&w(a)}).observe(document,{childList:!0,subtree:!0});function m(n){const c={};return n.integrity&&(c.integrity=n.integrity),n.referrerPolicy&&(c.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?c.credentials="include":n.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function w(n){if(n.ep)return;n.ep=!0;const c=m(n);fetch(n.href,c)}})();const R=3;let _=[],S=[],y=document.getElementById("connectionPath"),P=null,I=0;function F(){const p=document.querySelectorAll("#logo circle, #logo path");S=Array.from(p).map(l=>l.getAttribute("style").match(/fill: ([^;]+)/)[1])}function A(p,l,m){return p*(1-m)+l*m}function D(p){y.clientWidth;const l=y.clientHeight,w=document.getElementById("logo").getBoundingClientRect(),n=w.left+w.width/2;let c=Array(5).fill().map(()=>[]);for(let a=0;a<R;a++){const z=(a-R/2+.5)*10,x=n+z,f=n+z,d=5,v=150*(1+Math.sin(p*Math.PI)),b=1.5+p*2,u=p*Math.PI*2+a*Math.PI/R,s=[];for(let o=0;o<d;o++){const t=o/(d-1),r=Math.sin(t*Math.PI*b+u)*v,i=Math.cos(t*Math.PI*(b*1.5)+u)*(v*.5),g=A(x,f,t)+r*.5,L=A(0,l,t)+r+i;s.push({x:g,y:L}),c[o].push({x:g,y:L})}s[0].y=0,s[s.length-1].y=l;let h=`M ${s[0].x},${s[0].y}`;for(let o=0;o<s.length-1;o++){const t=s[o],r=s[o+1],i=(t.x+r.x)/2,g=(t.y+r.y)/2;h+=` Q ${t.x},${t.y} ${i},${g}`}h+=` L ${s[s.length-1].x},${s[s.length-1].y}`;let e=_[a];e||(e=document.createElementNS("http://www.w3.org/2000/svg","path"),_[a]=e,y.appendChild(e)),e.setAttribute("d",h),e.setAttribute("stroke",S[a%S.length]),e.setAttribute("fill","none"),e.setAttribute("stroke-width","1")}c.forEach((a,z)=>{if(a.length<2)return;let x=`M ${a[0].x},${a[0].y}`;for(let d=1;d<a.length-1;d++){const v=a[d-1],b=a[d],u=a[d+1],s=b.x-(u.x-v.x)*.2,h=b.y-(u.y-v.y)*.2,e=b.x+(u.x-v.x)*.2,o=b.y+(u.y-v.y)*.2;x+=` C ${s},${h} ${e},${o} ${u.x},${u.y}`}let f=y.querySelector(`#connecting-${z}`);f||(f=document.createElementNS("http://www.w3.org/2000/svg","path"),f.id=`connecting-${z}`,y.appendChild(f)),f.setAttribute("d",x),f.setAttribute("stroke",S[z%S.length])})}function U(){const p=window.scrollY/(document.documentElement.scrollHeight-window.innerHeight),m=A(I,p,.1);D(m),I=m,P=requestAnimationFrame(U)}F();y.style.position="fixed";y.style.top="0";y.style.left="0";y.style.pointerEvents="none";y.style.zIndex="1";U();window.addEventListener("pagehide",()=>{P&&cancelAnimationFrame(P)});(()=>{const m=`
    let gl, program;
